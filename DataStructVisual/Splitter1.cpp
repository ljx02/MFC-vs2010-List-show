...
int CSplitter1::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	m_wndSplitter1.CreateView(0,0,RUNTIME_CLASS(CDraw1), CSize(rect.Width(),rect.Height()), pContext);
	m_wndSplitter1.CreateView(1,0,RUNTIME_CLASS(CControl1), CSize(rect.Width(),rect.Height()), pContext);
	return 0;
}
...
void CSplitter1::OnSize(UINT nType, int cx, int cy) 
{
	...
	m_wndSplitter1.SetRowInfo(0, cy-(cy/1), 0);
	m_wndSplitter1.SetRowInfo(1, cy/1, 20);
	m_wndSplitter1.RecalcLayout();
	
}

void CSplitter1::OnCancelMode() 
{
	CView::OnCancelMode();
